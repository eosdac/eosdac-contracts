...
void daccustodian::updatebio(name cand, string bio) {
    ...
    const auto &reg_candidate = registered_candidates.get(cand, "ERR::UPDATEBIO_NOT_CURRENT_REG_CANDIDATE::Candidate is not already registered.");
...
void daccustodian::updatereqpay(name cand, asset requestedpay) {
    ...
    const auto &reg_candidate = registered_candidates.get(cand, "ERR::UPDATEREQPAY_NOT_CURRENT_REG_CANDIDATE::Candidate is not already registered.");

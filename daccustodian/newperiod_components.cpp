  ...
  if (meanAsset.amount > 0) {
    for (auto cust : custodians) {
      ...
      if (itrr != pendingPayReceiverSymbolIndex.end() && itrr->receiver == cust.cust_name &&
          itrr->quantity.get_extended_symbol() == total.get_extended_symbol() && itrr->due_date == time_point_sec{0}) {
...
void daccustodian::newperiode(string message, name dac_id) {
  ...
  eosio::action(
      eosio::permission_level{auth_account, "owner"_n}, get_self(), "runnewperiod"_n, make_tuple(message, dac_id))
      .send();
}
...
void daccustodian::runnewperiod(string message, name dac_id) {
  ...
  if (activation_account) {
    ...
    action(
        permission_level{activation_account, "notify"_n}, activation_account, "assertunlock"_n, std::make_tuple(dac_id))
        .send();
  } else {

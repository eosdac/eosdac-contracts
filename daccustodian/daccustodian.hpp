...
struct [[ eosio::table("config2"), eosio::contract("daccustodian") ]] contr_config {
  ...
  // Amount of token value in votes required to trigger the allow a new set of custodians to be set after the initial
...
struct [[ eosio::table("pendingpay2"), eosio::contract("daccustodian") ]] pay {
  ...
  time_point_sec due_date; // If there is a value here that indicates that a deferred transaction has been scheduled and
                           // that other new amounts should be added to this record.
...
typedef multi_index<"pendingpay2"_n, pay, indexed_by<"byreceiver"_n, const_mem_fun<pay, uint64_t, &pay::byreceiver>>,

...
ACTION dacescrow::transfer(name from, name to, asset quantity, string memo) {
  ...
  for (auto esc_itr = by_sender.lower_bound(from.value), end_itr = by_sender.upper_bound(from.value);
    ...
    if (esc_itr->ext_asset.quantity.amount == 0) {
      ...
      by_sender.modify(esc_itr, from, [&](escrow_info &e) { e.ext_asset = extended_asset{quantity, sending_code}; });
...
ACTION dacescrow::init(name sender, name receiver, name arb, time_point_sec expires, string memo, name ext_reference,
  ...
  auto by_sender = escrows.get_index<"bysender"_n>();
  check(escrows.find(ext_reference.value) == escrows.end(), "Already have an escrow with this external reference");

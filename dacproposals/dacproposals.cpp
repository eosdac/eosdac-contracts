...
ACTION dacproposals::createprop(name proposer, string title, string summary, name arbitrator, extended_asset pay_amount,
    string content_hash, uint64_t id, uint16_t category, uint32_t job_duration, uint32_t approval_duration,
    name dac_id) {
  ...
  check(proposals.find(id) == proposals.end(),
...
ACTION dacproposals::delegatecat(name custodian, uint64_t category, name delegatee_custodian, name dac_id) {
  ...
  if (vote_idx == by_cat_and_voter.end()) {
    ...
    by_cat_and_voter.modify(vote_idx, _self, [&](proposalvote &v) { v.delegatee = delegatee_custodian; });
  }
}
...
ACTION dacproposals::startwork(uint64_t proposal_id, name dac_id) {
  ...
  proposals.modify(prop, prop.proposer, [&](proposal &p) { p.state = ProposalStateWork_in_progress; });
  ...
  auto inittuple =
      make_tuple(treasury, prop.proposer, prop.arbitrator, time_now + prop.job_duration, memo, proposal_id, 0);

  eosio::action(eosio::permission_level{treasury, "escrow"_n}, escrow, "init"_n, inittuple).send();
...
ACTION dacproposals::completework(uint64_t proposal_id, name dac_id) {
  ...
  proposals.modify(prop, prop.proposer, [&](proposal &p) { p.state = ProposalStatePending_finalize; });
}
...
ACTION dacproposals::cancel(uint64_t proposal_id, name dac_id) {
  ...
  require_auth(prop.proposer);
  assertValidMember(prop.proposer, dac_id);
  // TODO: release the escrow if work has started.
  clearprop(prop, dac_id);
}

ACTION dacproposals::comment(
    name commenter, uint64_t proposal_id, string comment, string comment_category, name dac_id) {
...
ACTION dacproposals::updallprops(name dac_id) {
  ...
  for (auto props_itr : proposals) {
    ...
    deferredTrans.actions.emplace_back(eosio::action(
        eosio::permission_level{_self, "active"_n}, _self, "updpropvotes"_n, std::make_tuple(props_itr.key, dac_id)));
...
ACTION dacproposals::updpropvotes(uint64_t proposal_id, name dac_id) {
  ...
  if (prop.state != newPropState) {
    proposals.modify(prop, prop.proposer, [&](proposal &p) { p.state = newPropState; });
  }
}
...
void dacproposals::transferfunds(const proposal &prop, name dac_id) {
  ...
  eosio::action(eosio::permission_level{treasury, "escrow"_n}, escrow, "approve"_n, make_tuple(prop.key, treasury))
      .send();
...
int16_t dacproposals::count_votes(proposal prop, VoteType vote_type, name dac_id) {
  ...
  while (direct_vote_itr != by_voters.end() && direct_vote_itr->proposal_id == prop.key) {
  ...
  auto end_itr = std::set_difference(current_custodians.begin(), current_custodians.end(), voted_custodians.begin(),
      voted_custodians.end(), nonvoting_custodians.begin());
  ...
  for (auto custodian : nonvoting_custodians) {
    ...
    if (vote_idx != by_category.end() && vote_idx->category_id && vote_idx->delegatee && vote_idx->voter == custodian) {
...
//     const auto &regmem = reg_members.get(member.value, "ERR::GENERAL_REG_MEMBER_NOT_FOUND::Account is not registered
//     with members."); check((regmem.agreedterms != 0), "ERR::GENERAL_MEMBER_HAS_NOT_AGREED_TO_ANY_TERMS::Account has
//     not agreed to any terms"); auto latest_member_terms = (--memberterms.end()); check(latest_member_terms->version
//     == regmem.agreedterms, "ERR::GENERAL_MEMBER_HAS_NOT_AGREED_TO_LATEST_TERMS::Agreed terms isn't the latest.");
// }

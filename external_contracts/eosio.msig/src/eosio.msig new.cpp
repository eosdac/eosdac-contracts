...
namespace eosio {
    ...
    std::vector<permission_level> get_approvals_and_adjust_table(
        name self, name proposer, name proposal_name, Function &&table_op) {
    ...
    void multisig::propose(
        name proposer, name proposal_name, std::vector<permission_level> requested, ignore<transaction> trx) {
    ...
    void multisig::approve(name proposer, name proposal_name, permission_level level,
        ...
        if (prop.earliest_exec_time.has_value()) {
            if (!prop.earliest_exec_time->has_value()) {
                ...
                if (trx_is_authorized(get_approvals_and_adjust_table(get_self(), proposer, proposal_name, table_op),
    ...
    void multisig::exec(name proposer, name proposal_name, name executer) {
        ...
        bool ok = trx_is_authorized(
            get_approvals_and_adjust_table(get_self(), proposer, proposal_name, table_op), prop.packed_transaction);

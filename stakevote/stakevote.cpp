...
void stakevote::stakeobsv(vector<account_stake_delta> stake_deltas, name dac_id) {
  ...
  action(permission_level{get_self(), "notify"_n}, custodian_contract, "stakeobsv"_n, make_tuple(stake_deltas, dac_id))
      .send();
  ...
  for (auto asd : stake_deltas) {
    ...
    if (vw_itr != weights.end()) {
      weights.modify(vw_itr, same_payer, [&](auto &v) { v.weight += weight_delta; });
    } else {
